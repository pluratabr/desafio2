...
gera_percentual<-function(temp_tribunal,i){
  ...
  cat(nome_saida, "\n")
  nome_saida<-paste0("saida_perc_varas/",gsub(".json", ".csv", nome_saida))
  ...
  gera_df<- function(tribunal, sgt_assuntos){
    tribunal_assuntos<- tribunal %>% select(dadosBasicos.numero, dadosBasicos.assunto,
                                            ...
                                            descricao_classe,
                                            siglaTribunal,
                                            dadosBasicos.orgaoJulgador.nomeOrgao,dadosBasicos.orgaoJulgador.codigoOrgao)%>%
  ...
  assuntos_classe_percentual<- assuntos_classe%>%
    group_by(cod_assunto_classe,dadosBasicos.orgaoJulgador.codigoOrgao,dadosBasicos.orgaoJulgador.nomeOrgao,siglaTribunal)%>%
...
jf<-list.files("justica_eleitoral", full.names = T)
for (tr in jf ){
...
jus<-list.files("saida_perc_varas", full.names = T)
for(i in jus){
...
write_csv2(tribunal1_1, "tre_codigo_varas_classe_assunto.csv")
